...
fig4d <- function() {
  ...
  ndat_a2$preds <- stats::predict(m_a2, newdata = ndat_a2)

  figQuantile_Reg <- biblioCNDC %>%
  ggplot2::ggplot() +
    ggplot2::geom_point(ggplot2::aes(x = .data$minW, y = .data$A2), shape = 19, size = 1.7,
                        fill = "#3d3c3c", alpha = .8) +
    ggplot2:: geom_line(data = ndat_a2, ggplot2::aes(x = .data$minW, y = .data$preds),
                        color = "#E64B35FF", linetype = "solid", size = .5) +
