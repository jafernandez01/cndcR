...
#' @description
#' This function returns figure 3e and 3f in Fernandez et al.(2022): Sensitivity analyses for the
#' number of sampling times used to estimate a and b parameters of the CNDC in a case-study for
#' maize, presenting bias and uncertainty (precision).

fig3e_f <- function() {

fdataSens_3 <- eval(parse(text = "cndcR:::fdataSens_3"))

# Pre-process for plotting results ----------------------
## extract parameters for the full-model (boot 6) for comparisons
a1 <- as.numeric(fdataSens_3[fdataSens_3$Method == "Boot6" & fdataSens_3$Parameter == "A1", 5])
a2 <- as.numeric(fdataSens_3[fdataSens_3$Method == "Boot6" & fdataSens_3$Parameter == "A2", 5])

ci1 <- as.numeric(fdataSens_3[fdataSens_3$Method == "Boot6" & fdataSens_3$Parameter == "A1", 4]) -
  as.numeric(fdataSens_3[fdataSens_3$Method == "Boot6" & fdataSens_3$Parameter == "A1", 3])
ci2 <- as.numeric(fdataSens_3[fdataSens_3$Method == "Boot6" & fdataSens_3$Parameter == "A2", 4]) -
  as.numeric(fdataSens_3[fdataSens_3$Method == "Boot6" & fdataSens_3$Parameter == "A2", 3])

figSens_3 <- fdataSens_3  %>%
  dplyr::mutate(
    n = dplyr::case_when(
      Method == "Boot1" ~ 1,
      Method == "Boot2" ~ 2,
  ...
  # Figures 3e-f for sensitivity analysis ----------------------
ggplot2::ggplot(ggplot2::aes(x = n, y = .data$value, fill = .data$Parameter,
  ...
  ggplot2::xlab("Number of sampling times") +
