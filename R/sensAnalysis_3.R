...
## 3rd sensitivity analysis: *number of sampling times*

# Bootstrap procedure in which studies with *s* (where *s* = 1, 2, 3, 4, 5, and 6) number of
# samplings are sampled 100 times with replacement to create 100 new datasets.
...
# the power function of CNDC across sampling dates. Weakly-informative priors were defined following Makowski et al.
# (2020) and Ciampitti et al. (2021). To facilitate computation of the sensitivity analyses, the algorithm was run
# with three chains of 20,000 iterations each (10,000 discarded as tuning and burn-in periods). The statistical model
# was fitted using the rjags:: library.
...
job::job({
    ...
    set.seed(500)
    bootSamp_3 <- NULL
    bootSamp_3 <- foreach(i = seq(1, 6, 1), .verbose = T) %dopar% {
...
")
  ...
  }, import = "auto", title = "mcmc_no.samplings" )
...
# The effect of the sample size (i.e., number of sampling times) on the posterior medians was analyzed via
# the relative bias and uncertainty over the full model.

# Figures **3e-f** are obtained using ggplot:: library.

cl <- parallel::makeCluster(parallel::detectCores()[1] - 1) # the no. of cores - 1, to avoid overloading
doParallel::registerDoParallel(cl)

# Obtain final data_frame with posteriors for a and b
fdataSens_3 <- foreach(i = seq(1, 6, 1)) %dopar% { # for all the different no.sampling-times tested

  purrr::map2_dfr(
    bootSamp_3[[i]], seq(1, 100, 1), # map the extraction of parameters within the bootstrap samples
    ~ .x |>
      as.data.frame() |>
      dplyr::mutate(Imp = .y) |>
